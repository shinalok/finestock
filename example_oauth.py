...
def main():
    ...
    if app_key == "YOUR_APP_KEY" or app_secret == "YOUR_APP_SECRET":
        print(
            "APP_KEY/APP_SECRET 환경변수가 설정되지 않았습니다.\n"
            "  .env.example을 .env로 복사해 값을 채우거나,\n"
            "  PowerShell: $env:APP_KEY=\"...\"; $env:APP_SECRET=\"...\""
        )
        sys.exit(1)
    ...
    print(f"응답: {token}")
    if api.access_token:
        print(f"발급된 access_token: {api.access_token}")
        print(f"token_type: {api.token_type}")
        print("로그인 성공.")
    else:
